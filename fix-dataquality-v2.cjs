...
// Normalize CRLF -> LF so multiline template literals match
content = content.replace(/\r\n/g, '\n');

// ── Change 1: Add Layers + List to lucide-react imports ──────────────────────
if (!content.includes('Layers, List')) {
  content = content.replace(
  ...
  console.log('✓ Added Layers + List imports');
} else {
  console.log('- Imports already updated, skipping');
}

// ── Change 4: Update tabs array ───────────────────────────────────────────────
if (!content.includes(`{ id: 'segments'`)) {
  content = content.replace(
    `  const tabs = [
  ...
  console.log('- Tabs already updated, skipping');
}

// ── Change 5: Insert Segments + Variable Format tabs before Distributions ──────
if (!content.includes(`activeTab === 'segments' && (() => {`)) {
  const newTabsJSX = `
                  {/* Segments Tab */}
                  {activeTab === 'segments' && (() => {
...
`;
  ...
  console.log('✓ Inserted Segments + Variable Format tab JSX');
} else {
  console.log('- Tab JSX already present, skipping');
}
